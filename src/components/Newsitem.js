...
export class Newsitem extends Component {
    render() {
        ...
        let { title, description, publishedAt, imgurl, url, darkMode } = this.props


        return (
            <>
                
                <article className={`postcard ${ darkMode ? 'dark':'light'} blue`}>
                    <Link className="postcard__img_link" to={url} target='_blanck'>
                        <img className="postcard__img" src={!imgurl ? "https://picsum.photos/1000/1000" : imgurl} alt="card-img" />
                    </Link>
                    <div className={`postcard__text ${ darkMode ? '':'t-dark'} `}>
                        <h1 className="postcard__title blue"><a href={url} target='_blanck'>{title}</a></h1>
                        <div className="postcard__subtitle small">
                            <time >
                                <i className="fas fa-calendar-alt mx-2"></i>{new Date (publishedAt).toGMTString()}
                            </time>
                        </div>
                        <div className="postcard__bar"></div>
                        <div className="postcard__preview-txt">{description}</div>
                        <div className="postcard__tagbox ">
                            <Link className="tag__item" to={url} target='_blanck'><i className="fas fa-tag mx-2"></i>Read More</Link>
                        </div>
                    </div>
                </article>
